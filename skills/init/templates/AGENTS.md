...
模糊则升一档。大改动必须先讲结论和理由，**等人确认**后再写 PRD；确认前不要改产品代码。
...
场景匹配，或用户提到对应说法时，立刻读并执行该 Skill，不要只口头答应。
...
| 大改动、PRD、grill、grill-with-docs | `grill-with-docs` | `.agents/skills/grill-with-docs/SKILL.md` |
| 建任务、`.work/`、跨会话续跑、start-task | `start-task` | `.agents/skills/start-task/SKILL.md` |
