...
**提交**:
用户明确说「提交」时走 `commit-code`。只提交改动的业务代码，不能带任何配置文件和单测。说明格式为 `动作(模块):具体内容`。一次 commit 不等于归档。
...
**简化候选**:
`find-simplifications` 的产出：带调用方证据的删、折或降级项。没点名路径或模块就停；用户明确说「全仓」才全仓。列完清单就停，等人挑了再按候选分档落地。
...
**主动触发**:
用户说法对上才 Read 并执行。`find-simplifications` 属于这一类：简化、找死代码、收敛重复。
_Avoid_: 每次改代码都扫一遍, 写文档时顺手简化

**被动触发**:
模型进入该类工作就自己 Read，用户不必点名。`trim-cot-leakage` 属于这一类：正在写或改将进仓库的说明性文字时用。
_Avoid_: 用户手动指定去痕迹, 提交时全仓扫描, 把它做成和 commit-code 一样的口令 Skill
...
**边写边守**:
`trim-cot-leakage` 的用法：当场按规则落笔，不出审计表、不另起一遍 pass。改已有文字只动这次碰到的段落。
